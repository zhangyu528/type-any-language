...
### ✅ 4.4 [A.4] 登录后死循环跳回 /login

**现象**：在浏览器登录成功后，URL 显示 `/history` 一瞬间，又被踢回 `/login`。Backend API smoke test 完全正常（curl 能拿到 user），cookie 也设上了。

**根因**：`AuthProvider` 在 `app/layout.tsx`（根 layout），它的 `useEffect` **只在挂载时**调一次 `refresh()`（拿 user）。登录时序：
1. 用户进 `/` → AuthProvider 挂载 → apiMe → 没 cookie → user=null
2. 点登录 → `/login`（AuthProvider 没重新挂载）
3. 提交 → apiLogin → backend 设 cookie
4. `router.replace('/history')` → `/history` mount
5. 但 AuthProvider **没重新挂载**，user state 还是 null
6. `/history` 的 `useEffect` 看到 `!user` → `router.replace('/login')` —— 死循环

**修复**：login / signup 页面拿到 apiLogin/apiSignup 成功后，**主动调 `refresh()`** 把 user state 同步过来，然后再 navigate：
```ts
await apiLogin({ email, password });
await refresh();              // ← 同步 AuthProvider state
router.replace('/history');
```

**Commit**：`feat(auth): refresh AuthProvider state after login/signup before nav`（修复）
