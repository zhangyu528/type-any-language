...
| 配置文件 | `.env.db` | 无（shell env + `.secrets/`） |
| 入口脚本 | `scripts/ops/content/*.sh` | `scripts/ops/{dev-host,prod-host}/*.sh` |
| 需要 Python | 是 | 否 |
| 需要 AI key | 是 | 否 |
| 需要 TTS key | 是 | 否 |
| 写入 DB | 是 | 否（只读） |
...
```
1. 操作员提交新词库 CSV 到 content/source/vocabulary/
       │
       ▼
...
`content/runtime/init/01-content.sql` 是 bake 时生成的，由 postgres 镜像的 `/docker-entrypoint-initdb.d/` 在首次启动时执行。schema 内容由 `content/tools/cms/export_bundle.py` 的 `pg_dump --clean --if-exists` 输出保证幂等。
...
CSV 文件位于 `content/source/vocabulary/*.csv`（已提交到仓库，运维同学维护）。通过 `scripts/ops/content/content.sh sync`（底层 `content/tools/cms/import_vocab.py`）导入数据库。
...
./scripts/ops/content/content.sh sentences      # OpenAI 填句子
./scripts/ops/content/content.sh audio          # 腾讯云 TTS 烤 MP3
...
./scripts/ops/dev-host/run.sh start        # 首次会自动生成 .secrets/postgres_password
```

可选：`ALLOWED_ORIGINS=https://my.domain ./scripts/ops/dev-host/run.sh start`
...
- `.env.db` 全部 gitignore，仅 `.env.example.db` 入库。
...
├── .env.example.db                 # v3 CMS 模板（目标机不需要 .env）
│
├── backend/                         # FastAPI 纯读层
│   ├── app/
...
│   └── Dockerfile
│
├── content/                          # 内容服务 — 源 + CMS 工具链 + Postgres image 构建上下文
