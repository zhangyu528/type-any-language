...
#
# Purpose: replace local cms/.env + .dbcreds/ with a single source of
...
Typical workflow (dev host):
  cd ~/<repo>
  # dev doesn't need TENCENT_DB_* secrets — db is local docker postgres,
  # .dbcreds/db_password is auto-generated by ops/dev/setup.sh
  ./ops/dev/setup.sh           # 装 deps + 起本地 docker db
