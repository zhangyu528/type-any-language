...
# the registry.
#
# After build, run:  ./ops/prod/run.sh start
...
#   - CMS host:     run db/scripts/build.sh first.
...
if [ -z "$DB_USER" ] || [ -z "$DB_NAME" ]; then
    ...
    info "  → 重新跑 db/scripts/build.sh 烤一个带 label 的"
    exit 1
fi
