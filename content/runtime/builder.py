#!/usr/bin/env python3
"""
content/runtime/builder.py — assemble + build the content-baked db image.

Sister to content/runtime/Dockerfile: that file describes *how the
runtime image runs* (postgres:15-alpine wrapper, /docker-entrypoint-initdb.d,
shared-audio volume); this one describes *how it gets assembled*.

Two public entry points:

    assemble(bundle_dir, target="content/runtime")
        ...
        runtime build context:
            ...
            {bundle}/audio/*    -> {target}/seed/audio/
        Idempotent. Re-running with the same target + the same bundle
        produces the same staged inputs (rm -rf the destination dirs
...
(doctors the env, loads .env.db, picks a python, invokes this file).

Public API:
    assemble(bundle_dir, target) -> None
    build_image(target, tag, labels, build_args) -> str   # the tag

Usage:
    # from scripts/ops/content/bake_image.sh (after export_bundle.py)
    python content/runtime/builder.py \
...
- runtime/ now owns the question "what does it take to be buildable?"
...
def assemble(bundle_dir: Path, target: Path | None = None) -> tuple[Path, int]:
    ...
    seed_audio = target_path / "seed" / "audio"
    init_dir.mkdir(parents=True, exist_ok=True)
    seed_audio.parent.mkdir(parents=True, exist_ok=True)

    # dump.sql — single file, copy with metadata.
    shutil.copy2(dump_sql, init_dir / "01-content.sql")

    # audio/ — only if the bundle has one (some operator runs may have
    # emptied the audio dir yet, e.g. before generate_audio was run).
    src_audio = bundle / "audio"
    if src_audio.is_dir():
        shutil.rmtree(seed_audio, ignore_errors=True)
        seed_audio.mkdir(parents=True, exist_ok=True)
        # Trailing /. semantics (copy contents, not the audio/ dir itself).
        audio_count = 0
        for f in src_audio.iterdir():
            if f.is_file():
                shutil.copy2(f, seed_audio / f.name)
                audio_count += 1
    else:
        audio_count = 0

    return target_path, audio_count
...
def build_image(
    ...
    (django + openai + tencentcloud already pin psycopg2-binary).
    The docker CLI is the one tool the CMS host definitely has, and
    it's already on the path every bake_image.sh invocation uses.
    """
...
def compute_build_args(args: argparse.Namespace) -> dict[str, str]:
    """ARGS consumed by content/runtime/Dockerfile.
...
def main() -> int:
    ...
    target_path, audio_count = assemble(args.bundle, Path(args.target))
    ...
    if audio_count:
        print(f"[builder]   → seed/audio/ ({audio_count} file(s))")
    else:
        print(f"[builder]   → seed/audio/ (empty)")
