...
| **Target host** (dev or prod) | Content serving (3 services in one compose file) | devcli/ + ops/cvm/ (compose files at the repo root; see Layout rule below) | Docker (backend + frontend + db) |
...
./devcli/migrate.sh                              # apply all migrations from scratch
...
- **`devcli/migrate.sh`** — apply pending schema migrations to the live cloud
...
The DSN never lives in a docker image. The db password lives in
`.dbcreds/db_password` (chmod 600), generated by `ops/cvm/bootstrap.sh` on first
prod deploy. The backend container reads the password via compose's
`secrets:` block + `POSTGRES_PASSWORD_FILE`, then assembles `DATABASE_URL`
in its environment via `$(cat /run/secrets/db_password)`.
...
│
├── ops/                    # target-host operations + image build/release orchestrator (CVM + CI)
...
│   ├── cvm/                 # prod CVM runtime scripts + prod stack definition
│   │   ├── _common.sh       # shared setup + the compose() wrapper (image tags, registry, drift)
│   │   ├── bootstrap.sh     # one-time idempotent host prep (preflight + .dbcreds/db_password + data dir + nginx site)
│   │   ├── lifecycle.sh     # start / stop / restart (auto-pulls from DOCKER_REGISTRY)
│   │   ├── doctor.sh        # preflight (.dbcreds/db_password, images, port 80, drift)
...
│       └── e2e.sh
│
├── devcli/                # dev workstation scripts (host-native uvicorn + next dev; not an ops env)
...
│   ├── restart_backend.sh   # Windows-specific :8000 sweep helper
...
then assembles `DATABASE_URL` in the backend container via `$(cat /run/secrets/db_password)`.
...
bash dev start                       # = ./devcli/native.sh start (uvicorn + next dev on host)
bash dev stop
...
### Prod target host (RUN env)

The prod host is the RUN side — it pulls pre-built images from
`$DOCKER_REGISTRY` (or uses locally-loaded ones) and runs the containers.
Image construction is the BUILD side's job (see `Release` below).

```bash
# First-time, on the prod CVM:
./ops/cvm/bootstrap.sh                 # preflight + .dbcreds/db_password + sudo chown /var/lib/.../postgres
export CMS_HOST=user@cms.internal:/opt/type-any-language   # optional, auto-rsync
./ops/cvm/bootstrap.sh               # bootstrap auto-deploys + verifies on first run (or: ./ops/cvm/lifecycle.sh start)
...
`http://localhost` in the prod compose — override via shell env (shown above)
or edit the compose file directly. The db password is in `.dbcreds/db_password`
(chmod 600), generated by `bootstrap.sh`, mounted into the db container via
compose's `secrets:` block + `POSTGRES_PASSWORD_FILE`.
...
5. `db/scripts/import_staging.sh` reads the staging files and UPSERTs them into `vocabulary_libs` / `vocabulary_words` / `sentences` on the local docker postgres (`importer`). DATABASE_URL comes from compose env (when run via `docker compose run --rm backend`) or shell env.

**Runtime (target host):**
1. Prod `bootstrap.sh` (idempotent, one-time per host) generates `.dbcreds/db_password` (chmod 600) and creates `/var/lib/type-any-language/postgres` owned by UID 999.
2. Prod `deploy.sh` (THE go-live step, first-time + subsequent) pulls all 3 images, recreates db+backend+nginx. The db image's custom entrypoint auto-applies migrations + imports CMS content on container start.
3. Daily: `lifecycle.sh start|stop|restart` recreates containers; backend reads the db password via compose's `secrets:` block (`/run/secrets/db_password`) and connects to the docker postgres service over the internal network.
4. Frontend fetches a sentence, browser plays its MP3 directly from `sentences.audio_url` (a full Tencent Cloud COS URL). The backend exposes no `/audio` endpoint.
5. User submits answer → `validate_answer()` normalizes (lowercase, strip punctuation/spaces) and compares.
...
./devcli/migrate.sh        # source db/scripts/lib.sh; db_assemble_url; exec db/scripts/migrate.sh
```

`devcli/migrate.sh` requires `python3` + `psycopg2-binary` + `sqlalchemy`
...
3. Target hosts: `./ops/cvm/lifecycle.sh restart` — pulls the latest `english_backend` + `english_frontend` from the registry (backend picks up the schema change on next request)
...
**Merge rules**:
- Shared migrations (`0001`-`8999`) merge into master and stay. They are applied to every dev's db on next `./devcli/migrate.sh` (or equivalent).
...
`DATABASE_URL` is resolved on the db side (`db/scripts/migrate.sh` / `db/scripts/lib.sh::db_assemble_url`) in this order:

1. **Container (preferred — used by CVM via ops/cvm/docker-compose.yml)** — `docker compose run backend` injects `DATABASE_URL` via the compose `environment:` block (which evaluates `$(cat /run/secrets/db_password)` at compose-eval time). This is how `ops/prod/deploy.sh` runs migrations and the importer.
2. **Shell env** — `export DATABASE_URL=postgres://...` (temporary, e.g. CI / self-managed postgres / ad-hoc CLI).
3. **Assembled from POSTGRES_\*** — `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_HOST` / `POSTGRES_PORT` / `POSTGRES_DB` are URL-encoded and joined. Used when shell env is unset but the operator has the parts available.
4. **Error** — fails loudly with a hint pointing at the assembly entry point.

For prod, the canonical path is "container" — operators never need to assemble DSN manually. The db password comes from `.dbcreds/db_password` (chmod 600), generated by `ops/prod/bootstrap.sh`.
...
| `POSTGRES_PASSWORD` | (none — db side resolves via shell env, `DATABASE_URL`, or `.dbcreds/tencent_db_*` files) | `POSTGRES_PASSWORD=... ./db/scripts/init_schema.sh` |
...
- `BACKEND_IMAGE_TAG`, `FRONTEND_IMAGE_TAG` — image tag for backend/frontend. Default: `backend/VERSION` on both dev and prod hosts (the single per-segment file gates both the dev and prod image tags at the same value), same for `frontend/VERSION` (resolved by `ops/lib.sh`). Override per image, or set `IMAGE_TAG` to bump all images at once (CI use):
  ```bash
  IMAGE_TAG=v1.2.3 ./ops/cvm/lifecycle.sh start
  ```
