...
#
#   - db image         (content-baked, via scripts/ops/db/bake_image.sh)
...
#   db                 Just bake the db image (CMS content pipeline output).
#   dev                Build dev backend+frontend (requires db image present).
#   prod               Build prod backend+frontend (requires db image present).
#   -h | help          Show usage.
#
# The dev / prod app builds each need the db image's OCI labels
# (DB_USER / DB_NAME) — that's why the db bake runs first under `all`.
# If you already have a db image at the right tag, just run `dev` / `prod`
...
命令:
  ...
  db                只 bake db image (CMS 内容流水线产物)
...
示例:
  $0                          # 默认: db + dev + prod
  $0 all                      # 同上
  $0 dev                      # 只 dev 流 (假定 db image 已存在)
  $0 db                       # 只 bake db
  IMAGE_TAG=v0.3.0 $0 all     # 一次性覆盖所有 tag
  DB_IMAGE_TAG=v0.5.0 $0 all  # 只覆盖 db tag,dev/prod 走各自的 VERSION 文件

架构前提:
  - dev / prod 的 build_image.sh 需要 db image 的 OCI label (DB_USER / DB_NAME),
    所以 all / dev / prod 都假设 db image 已经在本地(或先跑过 db)。
...
cmd_all() {
    ...
    run_step "bake db image (content-baked)" \
