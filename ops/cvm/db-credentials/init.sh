#!/usr/bin/env bash
#
# ops/cvm/db-credentials/init.sh — initialize .dbcreds/db_password.
#
...
DB_CRED_DIR_NAME=".dbcreds"
DB_PASSWORD_FILE="${DB_CRED_DIR_NAME}/db_password"

info "=== secrets install ==="
mkdir -p "$PROJECT_DIR/.dbcreds"
chmod 700 "$PROJECT_DIR/.dbcreds"

if [ -f "$PROJECT_DIR/$DB_PASSWORD_FILE" ]; then
    ok ".dbcreds/db_password 已存在 — 跳过生成"
    chmod 600 "$PROJECT_DIR/$DB_PASSWORD_FILE"
    exit 0
fi

info "生成 .dbcreds/db_password (48 字符 URL-safe)..."
gen_secret 48 > "$PROJECT_DIR/$DB_PASSWORD_FILE"
chmod 600 "$PROJECT_DIR/$DB_PASSWORD_FILE"
ok ".dbcreds/db_password 已写入 (chmod 600)"
info "  ⚠️  这个文件不可 commit (.gitignore 已含 .dbcreds/)"
