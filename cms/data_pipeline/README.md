# cms/data_pipeline — content production Python modules
...
`PYTHONPATH=cms` and a `python3` interpreter).
...
| `import_vocab.py` | `python -m data_pipeline.import_vocab` | CSVs → `vocabulary_libs` + `vocabulary_words`. |
| `generate_sentences.py` | `python -m data_pipeline.generate_sentences` | OpenAI → `sentences` table (bucket fill). |
| `generate_audio.py` | `python -m data_pipeline.generate_audio` | Tencent TTS → MP3 + `sentences.audio_url`. |
| `export_bundle.py` | `python -m data_pipeline.export_bundle` | `pg_dump` + audio copy → staging bundle. Called by `bake_image.sh`; also exposed via `content.sh export` for inspection. |
...
PYTHONPATH=cms python3 -m data_pipeline.import_vocab

# As a script (also works — sys.path bootstrap in each file)
python3 cms/data_pipeline/import_vocab.py
```
...
single dep at the OS level. There is no separate `cms/requirements.txt`
on purpose: keep the dep set minimal and overlap with the runtime.)

## Workflow

```
cms/content/vocabulary/*.csv                    ← operator-maintained source
        ↓  content.sh sync
...
2. Add an `if __package__ in (None, "")` block at the top (copy from
   `import_vocab.py`).
3. Add a `cmd_<name>()` wrapper in `scripts/cms/content.sh`.
