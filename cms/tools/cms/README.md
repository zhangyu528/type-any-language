...
这些是 CMS 主机跑来生产内容的 **Python 工具**。通过 `cms/scripts/etl.sh <subcommand>` 调用(它会配 `PYTHONPATH=cms/tools` 和一个 `python3` 解释器)。
...
| `export_bundle.py` | lives at `db/scripts/export_bundle.py` now(db 的职责)。`etl.sh export` 是个透传 wrapper。 |
...
| 完整 ETL 编排 (含 import) | `./cms/scripts/run.sh` (会调 import_staging.sh 作为最后一步) |
...
cms/source/vocabulary/*.csv                  ← 运维维护的源 (E: Extract)
        ↓  etl.sh sync (import_vocab.py)
cms/.local/staging/vocabulary/<lib>.json     ← 中间产物
        ↓  etl.sh sentences (generate_sentences.py, OpenAI)
cms/.local/staging/sentences/<lib>.jsonl     ← (T: Transform, 一行一句)
        ↓  etl.sh audio (generate_audio.py, 腾讯云 TTS)
...
`cms/scripts/run.sh` 把上面从 sync 到 import_staging 一次性串起来。

要 bake 出 db image,跑完 `run.sh` 之后再接 `./db/scripts/build.sh`。

## 为什么同时支持模块 + 脚本两种形式?

每个脚本顶部那个 `if __package__ in (None, ""):` 块让两种调用都能 work。模块形式是 `etl.sh` 用的(更干净、可以做类型检查)。脚本形式是开发时或 notebook 里临时调用的。同一个文件,两种都行。
