...
/**
 * CurvedInput — port of https://reactbits.dev/components/curved-input
 *
 * 一个完整 "input + 弧形 placeholder + 提交按钮" 自包含的输入条。
 * 文字沿弧形 path 走,按钮嵌在条右端。本组件替代了"传统的 input + 旁边按钮"
 * 双元素组合 —— 把"输入 + 行动"做成一个原子。
 *
 * 适配点 vs 上游:
 * - `THEMES` / `SHADOWS` 闭包常量从外层抽出,作为模块顶层 const,避免每次
 *   render 重新创建。
 * - `useId` 保持 React 18 调用(上游就这样),uid 去冒号(确保 SVG 属性
 *   selector 合规)。
 * - `Icon` prop 接受 ReactNode,通过 cloneElement 居中注入到 SVG <g> 中心。
 * - `motion` 替换:CurvedInput 内部没有动画,只有跟随 caret 的弧线滚动和
 *   focus 状态切换(cursor 移到 SVG 上变成 text)。
 * - `prefers-reduced-motion`: 上游 SVG <animate> 闪烁 caret 改成 disabled
 *   (硬停掉 SVG 内置动画,我们没有引入 motion 的 caret blink)。
 * - 暴露 `Theme` / `TYPES` 给上层使用。
 */
...
// TAL Slate + Amber 调色板:深色模式配深空蓝,浅色模式配纸白。
// 暴露两个内置 theme("dark" / "light"),同时所有颜色字段都接受显式 prop 覆盖。
const THEMES: Record<Theme, ThemePalette> = {
  dark: {
    backgroundColor: '#0A1428', // slate-night 与 pages bg 同色
    textColor: '#E8EFF8',
    placeholderColor: '#7A91AE',
    borderColor: '#243859',
    buttonColor: '#5BA8F0', // slate action
    buttonTextColor: '#042C53',
    shadowColor: '#000000',
    focusRingColor: '#5BA8F0',
    iconColor: '#A8D0FF',
  },
  light: {
    backgroundColor: '#FFFFFF',
    textColor: '#0E1A2E', // slate-ink
    placeholderColor: '#7C8AA1',
    borderColor: '#B5C4D6',
    buttonColor: '#378ADD', // slate action
    buttonTextColor: '#FFFFFF',
    shadowColor: '#0C447C',
    focusRingColor: '#378ADD',
    iconColor: '#1D5FA5',
  },
};
...
const TYPES = ['text', 'search', 'tel', 'url', 'password', 'email', 'number'];
...
export default function CurvedInput({
  ...
  const focusRingColor = palette.focusRingColor;
  ...
  // Re-measure once webfonts finish loading — text width on the arc
  // depends on the actual font being used.
  useEffect(() => {
    let alive = true;
  ...
  const safeType = (SELECTABLE_TYPES as readonly string[]).includes(type)
    ? type
    : 'text';
  const inputMode =
  ...
  const svgStyle: CSSProperties | undefined = shadow
    ? {
        filter: `drop-shadow(0 ${shadow[0]}px ${shadow[1]}px ${hexToRgba(
          shColor,
          shadow[2]
        )})`,
      }
  ...
  if (geom && layout) {
    ...
    const chipFill = iconColor ?? palette.iconColor;
    ...
    content = (
      ...
      >
        ...
        <path
          ...
          d={bandPath}
          fill="none"
          stroke={focused ? focusRingColor : accentColor}
  ...
  return (
    <form
      ref={rootRef}
      className={`${styles.root} ${focused ? styles.focused : ''} ${className}`.trim()}
      style={{
...
export { TYPES as CURVED_INPUT_TYPES };
