...
# broken (no docker, no .env.db, schema migration crash). Best-effort on
...
#   1   hard failure on (a) / (b) / (c) / (f), OR .env.db missing / doctor fail
...
命令:
  ...
  doctor            prefight: .env.db + POSTGRES_PASSWORD + content.sh doctor + docker
  -h|--help|help    显示本帮助
...
注意:
  ...
  - 此脚本只在 CMS host 跑 (需要 .env.db + POSTGRES_PASSWORD)
EOF
}
