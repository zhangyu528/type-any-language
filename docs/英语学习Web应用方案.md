...
│  PostgreSQL ◀── etl.sh sentences (OpenAI)                          │


│        │                                                               │


│        ├── etl.sh audio (Tencent TTS)                             │
...
2. etl.sh sync → cms/.local/staging/vocabulary/<lib>.json（不连 DB）
...
3. etl.sh sentences → OpenAI 批量生成句子
...
CSV 文件位于 `cms/source/vocabulary/*.csv`（已提交到仓库，运维同学维护）。通过 `cms/scripts/etl.sh sync`（底层 `cms/tools/cms/import_vocab.py`）写入 `cms/.local/staging/vocabulary/<lib>.json`（**ETL 的 E 步骤，CMS 端只产文件,不连 DB**）。db 端通过 `db/scripts/import_staging.sh`（`dbtools.importer`）把 JSONL 文件 UPSERT 进 staging db。
...
./cms/scripts/etl.sh sentences      # OpenAI 填句子


./cms/scripts/etl.sh audio          # 腾讯云 TTS 烤 MP3
