...
这个目录是 **CMS 内容生产** 的根 —— 唯一在 CMS 主机上跑 Python 工具链、调用 OpenAI / 腾讯 TTS、把内容装入 `english_db_content` 镜像的位置。**db image 本身** 在仓库根的 [`../db/`](../db/) —— 拆分后,这是两个并列的子项目,各自有 Dockerfile。
...
└── tools/              # CMS 工具链 — 只活 CMS 主机,从不进 image
    ...
    └── cms/            # Python 包(env / manifest / import_vocab / generate_sentences / generate_audio / export_bundle / init_schema / migrations / storage)
```

仓库根的 `db/` 目录是 db image 的构建上下文(只关心 schema + sentences 表,不带 audio):
...
| `type-any-language.db.user` | `cms/.env` 里的 `$POSTGRES_USER` | `run.sh` → `DB_USER` |
| `type-any-language.db.name` | `cms/.env` 里的 `$POSTGRES_DB` | `run.sh` → `DB_NAME` |
| `type-any-language.content.version` | `cms/.env` 里的 `$DB_IMAGE_TAG` | `run.sh` 的日志行 |
