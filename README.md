...
```bash
# (一次性,首次) 在共享 docker postgres 上为本机创建 ROLE/DB + 写 DATABASE_URL
make db-bootstrap-dev            # 等价 ./ops/dev/setup.sh bootstrap

# 之后每次都跑(idempotent)
make dev-setup                   # 验 docker postgres 契约 + build dev 应用镜像
make dev-doctor                  # 前置检查(docker + compose + images + docker postgres 可达性)
make dev-start                   # 起来 — 自动用 DATABASE_URL
make dev-logs                    # 看日志
make dev-stop                    # 停
...
> 没 `DATABASE_URL` 也没 `DATABASE_URL` 环境变量,`make dev-doctor` 会提示跑 `make db-bootstrap-dev`(docker postgres 主机)或 `export DATABASE_URL=postgres://...`(自管 / CI)。
...
2. **docker postgres 契约** —— 验证 `DATABASE_URL` 存在(docker postgres 主机)或 `DATABASE_URL` 在 shell env(自管 / CI)。任一就绪即可
