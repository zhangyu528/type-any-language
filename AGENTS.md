...
`.secrets/db_password` (chmod 600), generated by `ops/cvm/bootstrap.sh` on first
...
│   │   ├── bootstrap.sh     # one-time idempotent host prep (preflight + .secrets/db_password + data dir + nginx site)
...
./ops/cvm/bootstrap.sh                 # preflight + .secrets/db_password + sudo chown /var/lib/.../postgres
