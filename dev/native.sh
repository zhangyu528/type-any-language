#!/usr/bin/env bash
#
# dev/native.sh — host-native dev loop (default).
#
...
cmd_status() {
    ...
    if dev_db_is_up; then
        ...
        warn "db (docker compose): not up — run: ./dev/native.sh start (auto-heals)"
    fi
}
...
想测 docker image 路径:
  make dev-docker-start     # = ./dev/lifecycle.sh start
EOF
}
