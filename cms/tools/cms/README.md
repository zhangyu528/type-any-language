...
cms/.local/staging/sentences/<lib>.jsonl     ← (T: Transform, 一行一句)
        ↓  staging.sh audio (generate_audio.py, 腾讯云 TTS)
...
每个脚本顶部那个 `if __package__ in (None, ""):` 块让两种调用都能 work。模块形式是 `staging.sh` 用的(更干净、可以做类型检查)。脚本形式是开发时或 notebook 里临时调用的。同一个文件,两种都行。
