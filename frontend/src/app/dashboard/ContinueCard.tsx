...
export interface ContinueCardProps {
  ...
  onPickLib: () => void;
  /** CTA handler when there is NO current course: jump to 发现页 to add one.
   *  取代旧的「开 LibPicker」行为——无课时直接进入发现页挑课，而非弹选库器。 */
  onAddCourse?: () => void;
...
export default function ContinueCard({
  ...
  // 无当前课(空态) → 「添加课程」（跳发现页）。卡片不再单独显示 title。
  const cta = selId
