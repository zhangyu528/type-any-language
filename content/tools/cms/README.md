# content/tools/cms —— 内容生产的 Python 模块

这些是 CMS 主机跑来生产内容的 **Python 工具**。通过 `scripts/ops/content/content.sh <subcommand>` 调用(它会配 `PYTHONPATH=content/tools` 和一个 `python3` 解释器)。
...
```sh
# 作为模块(推荐 —— 用相对 import)
PYTHONPATH=content/tools python3 -m cms.import_vocab

# 作为脚本(也可以 —— 每个文件里有 sys.path bootstrap)
python3 content/tools/cms/import_vocab.py
```
...
(同一个 psycopg2 也被 `backend/requirements.txt` 用了 —— 操作系统层面是同一个依赖。故意没有单独的 `content/tools/requirements.txt`:让依赖集尽量精简,跟运行时重合。)

## 流水线

```
content/source/vocabulary/*.csv                  ← 运维维护的源
        ↓  content.sh sync
...
1. 在 `content/tools/cms/<name>.py` 放一个新文件,顶部加一个 `if __package__ in (None, "")` 块(从 `import_vocab.py` 复制)。
2. 在 `scripts/ops/content/content.sh` 加一个 `cmd_<name>()` 包装。
