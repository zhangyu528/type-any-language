...
`.secrets/db_password` (chmod 600), generated by `ops/prod/prepare.sh` on first
...
│       ├── prepare.sh        # host-level prep: preflight + .secrets/db_password + sudo chown /var/lib/.../postgres
...
./ops/prod/prepare.sh                 # preflight + .secrets/db_password + sudo chown /var/lib/.../postgres
...
(chmod 600), generated by `prepare.sh`, mounted into the db container via
...
**Runtime (target host):**
1. Prod `prepare.sh` (idempotent, one-time per host) generates `.secrets/db_password` (chmod 600) and creates `/var/lib/type-any-language/postgres` owned by UID 999.
...
For prod, the canonical path is "container" — operators never need to assemble DSN manually. The db password comes from `.secrets/db_password` (chmod 600), generated by `ops/prod/prepare.sh`.
