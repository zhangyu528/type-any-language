...
`.dbcreds/db_password` (chmod 600), generated by `ops/prod/bootstrap.sh` on first
...
│   └── prod/                # prod target host — RUN env (no migrate/watch; no build)
│       ├── _common.sh
...
│       ├── bootstrap.sh        # host-level prep: preflight + .dbcreds/db_password + sudo chown /var/lib/.../postgres
│       ├── deploy.sh         # THE go-live step. First-time + subsequent. db image entrypoint auto-inits.
│       ├── logs.sh
│       ├── build_image.sh    # BUILD env: build english_backend + english_frontend
│       ├── push_image.sh     # BUILD env: push prod backend+frontend to DOCKER_REGISTRY
...
bash dev start                       # = ./ops/dev/native.sh start (uvicorn + next dev on host)
bash dev stop
...
./ops/prod/bootstrap.sh                 # preflight + .dbcreds/db_password + sudo chown /var/lib/.../postgres
export CMS_HOST=user@cms.internal:/opt/type-any-language   # optional, auto-rsync
./ops/prod/deploy.sh               # fetch content + start db + migrate + import + start + verify
ALLOWED_ORIGINS=https://my.domain ./ops/prod/lifecycle.sh status
...
```bash
# On the build host:
./ops/prod/release.sh prod v0.3.0 -y        # bump VERSION + build + push to $DOCKER_REGISTRY
# or step-by-step:
./ops/prod/build/image.sh               # build english_backend + english_frontend
./ops/prod/build/push.sh -y             # push them to $DOCKER_REGISTRY
```
...
./ops/prod/release.sh prod v0.4.0 -y
# → english_backend:v0.4.0  (and pushed to ${DOCKER_REGISTRY} if set)
```

For a full release (bump + build + push), use `ops/prod/release.sh prod X.Y.Z` instead of running the build scripts individually — see "Release flow" below.
...
`ops/prod/release.sh` is the single point of release orchestration. `cmd_prod` bumps VERSION, commits, then builds + tags + pushes prod images.
...
./ops/prod/release.sh prod v0.3.0 -y

# Re-publish current VERSION (no bump)
./ops/prod/release.sh dev
```
...
./ops/prod/release.sh prod v0.3.0 -y    # bump backend/VERSION + frontend/VERSION + build + push prod b/f
git push
...
`ops/prod/release.sh` now bumps a single file per stream (dev bumps just
...
# Make wrapper (root Makefile 已删除,直接调脚本):
./db/scripts/next_migration_prefix.sh
```
...
1. **Container (preferred)** — `docker compose run backend` injects `DATABASE_URL` via the compose `environment:` block (which evaluates `$(cat /run/secrets/db_password)` at compose-eval time). This is how `ops/prod/deploy.sh` runs migrations and the importer.
