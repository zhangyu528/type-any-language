...
`.secrets/db_password` (chmod 600), generated by `ops/prod/bootstrap.sh` on first
...
│       ├── bootstrap.sh        # host-level prep: preflight + .secrets/db_password + sudo chown /var/lib/.../postgres
...
The runtime `docker-compose.yml` mounts the host-side `.secrets/db_password`
...
./ops/prod/bootstrap.sh                 # preflight + .secrets/db_password + sudo chown /var/lib/.../postgres
...
or edit the compose file directly. The db password is in `.secrets/db_password`
...
**Runtime (target host):**
1. Prod `bootstrap.sh` (idempotent, one-time per host) generates `.secrets/db_password` (chmod 600) and creates `/var/lib/type-any-language/postgres` owned by UID 999.
...
For prod, the canonical path is "container" — operators never need to assemble DSN manually. The db password comes from `.secrets/db_password` (chmod 600), generated by `ops/prod/bootstrap.sh`.
