...
Resolution chain (priority order):
  ...
  2. Assembled from POSTGRES_USER / POSTGRES_DB / POSTGRES_HOST /
     POSTGRES_PORT / POSTGRES_PASSWORD — falls back to .dbcreds/
...
# Where .dbcreds/postgres_password lives if it exists. This was the
...
def resolve_database_url() -> str:
    ...
    + POSTGRES_PASSWORD (from env or .dbcreds/postgres_password).
    ...
    if not pw:
        # Last resort: try .dbcreds/postgres_password (legacy cloud-db
        # storage, kept as a defensive fallback for self-hosted /
        # ad-hoc CLI use).
        dbcreds = find_project_root() / _DB_CRED_DIR_NAME / "postgres_password"
        if secrets.is_file():
    ...
    if not pw:
        sys.exit(
            ...
            "either export POSTGRES_PASSWORD or create .dbcreds/postgres_password."
        )
