...
For each sentence where audio_url is empty, call Tencent Cloud TTS,
write the resulting MP3 to the configured Storage (local FS or Tencent
COS — see cms.storage), and UPDATE sentences.audio_url to the
storage's public_url() for the key. The frontend reads this URL and
the browser streams the audio (no backend /audio endpoint).

Idempotent on rerun:
  - Skips sentences whose audio_url is already set AND the storage key
...
CLOUD_* credentials must be all-or-nothing when CLOUD_PROVIDER is not
local_fs (see env.py::require_cloud).
"""
import argparse
import hashlib
import sys
...
if __package__ in (None, ""):
    sys.path.insert(0, str(__file__).resolve().parent.parent.parent.parent)
...
def main() -> None:
    ...
    setup_env()
    cfg = load_config()
    cfg.require_tencent()  # raise clearly if TENCENT_* unset, with a pointer at cms/.env
    ...
    if isinstance(storage, LocalFsStorage):
        ...
        print(f"[generate_audio] bucket:     {storage.bucket}")
    ...
    with psycopg2.connect(cfg.database_url) as conn:
        ...
        for i, (sid, text, _) in enumerate(rows, 1):
            ...
            try:
                # Skip if storage already has the file (idempotency).
                # Always rewrite the audio_url though, in case the
                # storage provider (e.g. COS bucket name) changed
                # since last run.
                if storage.exists(key) and not args.force:
                    mark_audio_url(conn, sid, storage.public_url(key))
                    conn.commit()
                    skip += 1
                ...
                if i % 10 == 0 or i == len(rows):
                    ...
                    print(f"  [{i}/{len(rows)}] ok={ok} skip={skip} fail={fail}  ({rate:.1f}/s)")
        ...
        print()
        print(f"[generate_audio] done — ok={ok} skip={skip} fail={fail}  ({time.time()-start:.1f}s)")
