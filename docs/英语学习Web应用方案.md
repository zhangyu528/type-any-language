...
| 入口脚本 | `scripts/ops/cms/*.sh` | `scripts/ops/{dev-host,prod-host}/*.sh` |
| 需要 Python | 是 | 否 |
| 需要 AI key | 是 | 否 |
| 需要 TTS key | 是 | 否 |
| 写入 DB | 是 | 否（只读） |
...
CSV 文件位于 `cms/source/vocabulary/*.csv`（已提交到仓库，运维同学维护）。通过 `scripts/ops/cms/content.sh sync`（底层 `cms/tools/cms/import_vocab.py`）导入数据库。
...
./scripts/ops/cms/content.sh sentences      # OpenAI 填句子
./scripts/ops/cms/content.sh audio          # 腾讯云 TTS 烤 MP3
